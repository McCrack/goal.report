...
if(defined("ARG_3")){
	if(is_numeric(ARG_3)){
		$post = $mySQL->getRow("
		SELECT
		PageID,ID,SetID,language,published,header,subheader,landscape,portrait,category,created,video,Ads,
				...
				<div id="feed">
					...
					<a class="snippet" href="/blogger/<?=$row['language']?>/<?=$page?>/<?=$row['PageID']?>">
						<div class="preview">
							...
							<?else:?>
							<video src="<?=$row['video']?>" poster="<?=$row['landscape']?>" muted></video>
							<?endif?>
						</div>
						<div class="header"><?=$row['header']?></div>
						<div class="options">
			...
			<section>
				<div class="tabs">
					...
					<form id="metadata" class="tab body-bg light-txt" autocomplete="off">
						...
						<!-- COVERS -->
						<input id="landscape-tab" type="radio" name="cover-tab" hidden checked>
						<label for="landscape-tab">Landscape</label>
						<input id="portrait-tab" type="radio" name="cover-tab" hidden>
						...
						<div class="black-bg" align="right">
							...
							<div class="select" title="template" data-translate="title">
								<select name="template" class="active-txt"> 
									<option selected value="<?=$post['subtemplate']?>"><?=$post['subtemplate']?></option>
									<?foreach(glob("../".BASE_FOLDER."/themes/".$cng->theme."/includes/post/*.html") as $file):
									$file = pathinfo($file)['filename'];
									if($file==$post['subtemplate']) continue?>
