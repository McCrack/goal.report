<?switch(ARG_2){
	...
	case "create-post":
		...
		$mySQL->inquiry("INSERT INTO gb_blogcontent SET PageID={int}, UserID={int}, subtemplate='default'", $PageID,USER_ID);
	...
	case "clone-post":
		...
		SELECT
			ID,SetID,landscape,portrait,video,category,created,Ads,
	...
	case "save-metadata":
		...
		$tags = [];
		foreach($p['keywords'] as $tag) $tags[] = translite($tag);

		if(!empty($keywords)){
			foreach($keywords as $key){
				$mySQL->inquiry("INSERT INTO gb_keywords (KeyWORD,tag) VALUES ({str},{str}) ON DUPLICATE KEY UPDATE rating=rating+1", $key,translite($key));
			}
		...
		$IDs = $mySQL->getGroup("SELECT KeyID FROM gb_keywords WHERE tag IN ('".implode("','", $tags)."')")['KeyID'];
		...
		foreach(["ID","header","subheader","landscape","portrait","video","category","created","UserID","subtemplate","published"] as $key){
			if($info[$key]==$p[$key]){
